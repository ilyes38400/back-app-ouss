...
class WorkoutController extends Controller
{
    ...
    public function getList(Request $request)
    {
        ...
        if( $request->has('program_type') && isset($request->program_type) ) {
            $workout = $workout->where('program_type', request('program_type'));
        }

        if( $request->has('is_monthly_program') && isset($request->is_monthly_program) ) {
            $workout = $workout->where('is_monthly_program', request('is_monthly_program'));
        }
        ...
        // Ajouter les informations d'accès pour chaque workout
        $userId = $request->input('user_id') ?? auth()->id();
        if ($userId) {
            foreach ($workout as $workoutItem) {
                $this->addAccessInfo($workoutItem, $userId);
            }
        }
        ...
        $response = [
            'pagination'    => json_pagination_response($workout),
            'data'          => $items,
        ];
    ...
    public function getDetail(Request $request)
    {
        ...
        $userId = $request->input('user_id') ?? auth()->id();
        if ($userId && !$this->hasAccessToProgram($workout, $userId)) {
            return response()->json([
                'status' => false,
                'message' => 'Accès refusé à ce programme',
                'access_required' => $workout->program_type === 'paid' ? 'purchase' : 'subscription'
            ], 403);
        }
        ...
        if ($userId) {
            $accessInfo = $this->addAccessInfo($workout, $userId);
            $workout->user_has_access = $accessInfo->user_has_access;
            $workout->access_reason = $accessInfo->access_reason;
            $workout->requires_purchase = $accessInfo->requires_purchase;
            $workout->requires_subscription = $accessInfo->requires_subscription;
        }
    ...
    public function getDetailWithAccess(Request $request)
    {
        ...
        $userId = $request->input('user_id') ?? auth()->id();
        if ($userId) {
            $this->addAccessInfo($workout, $userId);
        }
    ...
    public function getMonthlyPrograms(Request $request)
    {
        $workout = Workout::where('status', 'active')->monthlyProgram();
