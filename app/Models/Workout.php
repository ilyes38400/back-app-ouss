...
class Workout extends Model implements HasMedia
{
    ...
    protected $table = 'app_workouts';


    protected $fillable = [ 'title', 'description', 'status', 'workout_type_id', 'level_id', 'program_type', 'price', 'is_monthly_program' ];

    protected $casts = [
        'workout_type_id'   => 'integer',
        'level_id'          => 'integer',
        'price'             => 'decimal:2',
        'is_monthly_program' => 'boolean',
    ];
    ...
    public function scopeMonthlyProgram($query)
    {
        return $query->where('is_monthly_program', true);
    }
